...
import mimer29or40.foremanfx.gui.graph.element.old.GhostNodeElementOld;
import mimer29or40.foremanfx.gui.graph.element.old.GraphElementOld;
import mimer29or40.foremanfx.model.Item;

import java.util.*;

public class ProductionGraphPaneOld extends Pane
{
    public  HashSet<GraphElementOld> elements = new HashSet<>();
    public  ProductionGraph          graph    = new ProductionGraph();
    private List<Item>               demands  = new ArrayList<>();
    public  boolean isBeingDragged;
    private Point2D lastMouseDragPoint;
    public  Point2D viewOffset;
    public float viewScale = 1F;
    private GraphElementOld draggedElement;
    private Point2D         dragStartScreenPoint;
    public  boolean                         clickHasBecomeDrag      = false;
    public  Queue<Tooltip>                  tooltipsToDraw          = new LinkedList();
    private Font                            size10Font              = new Font("GenericSansSerif", 10);
    public  boolean                         showAssemblers          = false;
    public  boolean                         showMiners              = false;
    public  boolean                         showInserters           = false;
    //    StringFormatter stringFormat = new StringFormatter();
    public  GhostNodeElementOld             ghostDragElement        = null;
    public  HashSet<FloatingTooltipControl> floatingTooltipControls = new HashSet<>();
    private Rectangle2D graphBounds;
    private final GraphicsContext graphics = graph.getGraphicsContext2D();
    private final DoubleProperty  scale    = new SimpleDoubleProperty(1.0);
    private final DoubleProperty  height   = new SimpleDoubleProperty(getHeight())
    {
        @Override
    ...
    private final DoubleProperty  width    = new SimpleDoubleProperty(getHeight())
    {
        @Override
        protected void invalidated()
        {
    ...
    public ProductionGraphPaneOld()
    {
        super();
//        initializeComponents();
        viewOffset = new Point2D(getWidth() / -2, getHeight() / -2);
...
//    {
//        for (NodeElement node : elements.OfType<NodeElement>().ToList())
//        {
//            node.Update();
//        }
//        Invalidate();
...
//    {
//        Elements.RemoveWhere(e => e is LinkElement && !Graph.GetAllNodeLinks().Contains((e as LinkElement)
// .DisplayedLink));
...
//        {
//            clickedElement.MouseDown(Point.Add(ScreenToGraph(e.Location), new Size(-clickedElement.X,
// -clickedElement.Y)), e.Button);
//        }
//
...
//        {
//            Point dragDiff = Point.Add(Control.MousePosition, new Size(-dragStartScreenPoint.X,
// -dragStartScreenPoint.Y));
//            if (dragDiff.X * dragDiff.X + dragDiff.Y * dragDiff.Y > 9) //Only drag if the mouse has moved more than
// three pixels. This avoids dragging when the user is trying to click.
//            {
//                clickHasBecomeDrag = true;
//                DraggedElement.Dragged(Point.Add(ScreenToGraph(e.Location), new Size(-DraggedElement.X,
// -DraggedElement.Y)));
//            }
//        }
//
...
//        if (node != null)
//        {
//            foreach (NodeLink link in node.DisplayedNode.InputLinks.ToList().Union(node.DisplayedNode.OutputLinks
// .ToList()))
//            {
...
//                    {
//                        optionList.Add(new RecipeChooserControl(recipe, String.Format("Create '{0}' recipe node",
// recipe.FriendlyName), recipe.FriendlyName));
//                    }
//                }
...
//                    {
//                        newElement = new NodeElement(RecipeNode.Create((c as RecipeChooserControl).DisplayedRecipe,
// this.Graph), this);
//                    }
...
//                    Graph.UpdateNodeValues();
//                    newElement.Update();
//                    newElement.Location = Point.Add(location, new Size(-newElement.Width / 2,
// -newElement.Height / 2));
//                }
//                });
//            }
...
//        info.AddValue("EnabledAssemblers", DataCache.Assemblers.Values.Where(a => a.Enabled).Select<Assembler,
// String>(a => a.Name));
//        info.AddValue("EnabledMiners", DataCache.Miners.Values.Where(m => m.Enabled).Select<Miner,
// String>(m => m.Name));
//        info.AddValue("EnabledModules", DataCache.Modules.Values.Where(m => m.Enabled).Select<Module,
// String>(m => m.Name));
